...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const inputEventFunction = () => {
    ...
    if (keywords.length === 1 && keywords[0] === '') {
      ...
      resultContent.innerHTML = '<div class="no-records">There is no records.</div>';
    } else {
      ...
      if (resultItems.length === 1) {
        ...
        <div class="search-result-complete">
          <div class="search-result-number">There is only one record.</div>
          <ul class="search-result-list">${resultItems.map(result => result.item).join('')}</ul>
        </div>`;
      } else {
        resultContent.innerHTML = `
        <div class="search-result-complete">
          <div class="search-result-number">There are ${resultItems.length} records.</div>
          <ul class="search-result-list">${resultItems.map(result => result.item).join('')}</ul>
        </div>`;
      }
